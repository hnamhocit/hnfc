...
export default function SettingsPage() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      setError(e?.message ?? tCommon("failedToSave"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card text-card-foreground shadow-sm p-6 space-y-5">
        ...
        <div className="space-y-2">
          <Label htmlFor="photoURL">{tSettings("photoUrl")}</Label>
          <Input
            id="photoURL"
            value={photoURL}
            onChange={(e) => setPhotoURL(e.target.value)}
            placeholder={tSettings("photoUrlPlaceholder")}
            className="bg-background border-input text-foreground"
          />
        ...
        <div className="flex justify-end gap-2">
          <Button asChild variant="secondary">
            <Link href="/me/profile">{tCommon("cancel")}</Link>
          </Button>
